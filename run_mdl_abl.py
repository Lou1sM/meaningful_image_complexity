...
if sys.argv[1] == 'im':
    ...
    for fname in im_ims_used:
        ...
        ims_list.append(no_mdls)

    ims_arr = np.array(ims_list)
    df = pd.DataFrame({'mean':ims_arr.mean(), 'var':ims_arr.var(), 'std':ims_arr.std(), 'raw':ims_list})
    df.to_csv('experiments/main_run/im_no_mdl_abl.csv')
    ...
    for i,im_used_idx in enumerate(cifar_ims_used[:2]):
        print(i)
        im = torch_dset.data[int(im_used_idx)]
        im = np.array(Image.fromarray(im).resize((224,224)))/255
        no_mdls, _, _ = comp_meas.interpret(im)
        results_list.append(no_mdls)

    results_arr = np.array(results_list)
    df = pd.DataFrame({'mean':results_arr.mean(), 'var':results_arr.var(), 'std':results_arr.std(), 'raw':results_list})
    df.to_csv('experiments/main_run/cifar_no_mdl_abl.csv')

elif sys.argv[1] == 'dtd':
    # dtd
    with open(f'experiments/main_run/dtd_ARGS.txt') as f:
    ...
    comp_meas.is_mdl_abl = True
    ims_list = []
    for fname in listdir('dtd/suitable'):
    ...
    ims_arr = np.array(ims_list)
    df = pd.DataFrame({'mean':ims_arr.mean(), 'var':ims_arr.var(), 'std':ims_arr.std(), 'raw':ims_list})
    df.to_csv('experiments/main_run/dtd_no_mdl_abl.csv')
    ...
    ims_arr = np.array(ims_list)
    df = pd.DataFrame({'mean':ims_arr.mean(), 'var':ims_arr.var(), 'std':ims_arr.std(), 'raw':ims_list})
    df.to_csv('experiments/main_run/mnist_no_mdl_abl.csv')
    ...
    ims_arr = np.array(ims_list)
    df = pd.DataFrame({'mean':ims_arr.mean(), 'var':ims_arr.var(), 'std':ims_arr.std(), 'raw':ims_list})
    df.to_csv('experiments/main_run/stripes_no_mdl_abl.csv')
    ...
    ims_arr = np.array(ims_list)
    df = pd.DataFrame({'mean':ims_arr.mean(), 'var':ims_arr.var(), 'std':ims_arr.std(), 'raw':ims_list})
    df.to_csv('experiments/main_run/halves_no_mdl_abl.csv')
    ...
    ims_arr = np.array(ims_list)
    df = pd.DataFrame({'mean':ims_arr.mean(), 'var':ims_arr.var(), 'std':ims_arr.std(), 'raw':ims_list})
    df.to_csv('experiments/main_run/rand_no_mdl_abl.csv')
